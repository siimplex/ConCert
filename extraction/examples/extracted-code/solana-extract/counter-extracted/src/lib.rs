...
impl<'a> ContractInstruction<'a> {
  pub fn deserialize(input: &[u8], _arena: &'a bumpalo::Bump) -> Result<ContractInstruction<'a>, ProgramError> {
...
fn new_iter<A>(&'a self) -> &'a dyn Fn(&[A]) -> Iter<'_, A> { self.alloc(move |l| l.iter()) }

fn iter_next2(&'a self) -> impl Fn(SliceAccountInformation<'a>) -> &'a dyn Fn(&mut usize) -> Result<&'a AccountInfo<'a>, ProgramError> { move |slc : SliceAccountInformation| self.alloc( 
    move |index| { let item = slc.get(*index).ok_or(ProgramError::AccountBorrowFailed); *index += 1; item } 
 ) }
...
fn get_account_address(&'a self) -> &'a dyn Fn(&'a AccountInfo<'a>) -> Pubkey { self.alloc(move |acc| *acc.key) }

fn ser_data_account<T : BorshSerialize>(&'a self) -> impl Fn(()) -> &'a dyn Fn(&'a T) -> &'a dyn Fn(&'a AccountInfo<'a>) -> Result<(), ProgramError> { move |_| { self.alloc( move |v| self.alloc( move |acc| { match (*v).serialize(&mut &mut (*acc).data.borrow_mut()[..]) { Ok(_) => Ok(()), Err(_) => Err(ProgramError::BorshIoError(String::from("Serialization error")))}}))}}


fn counter_init(&'a self, accounts: SliceAccountInformation<'a>, init_value: i64) -> Result<(), ProgramError> {
  let mut index = 0;
  match self.iter_next2()(
          accounts)(&mut index) {
    Ok(t) => {
      match self.iter_next2()(
        accounts)(&mut index) {
...
fn counter_init__curried(&'a self) -> &'a dyn Fn(SliceAccountInformation<'a>) -> &'a dyn Fn(i64) -> Result<(), ProgramError> {
...
fn counter__curried(&'a self) -> &'a dyn Fn(SliceAccountInformation<'a>) -> Result<(), ProgramError> {
  self.closure(move |accounts| {
    self.counter(
      accounts)
  })
...
fn counter_process(&'a self, chain: &'a Chain<'a>, accounts: &'a [AccountInfo<'a>], inst: Option<&'a ContractInstruction<'a>>) -> Result<(), ProgramError> {
  match inst {
    Some(m) => {
      self.counter(
        accounts)
