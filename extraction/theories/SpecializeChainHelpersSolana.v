...
Definition ChainHelpers_kername : kername :=
  <%% @ChainHelpers %%>.
    ...
    | ConstantDecl cst =>
      let remove := match cst_type cst with
                    | tProd _ (tInd ind _) _ =>
                      eq_kername (inductive_mind ind) (ChainHelpers_kername)
                    | _ => false
                    end in
...
Definition axiomatized_ChainHelpers := 0.

Definition axiomatized_ChainHelpers_kername : kername :=
  <%% axiomatized_ChainHelpers %%>.

Definition axiomatized_ChainHelpers_decl : global_decl :=
  ConstantDecl
    {| cst_type :=
          tInd
            {| inductive_mind := ChainHelpers_kername;
              inductive_ind := 0; |}
            [];
        cst_body := None;
...
Fixpoint specialize_env_rev (Σ : global_env) : result global_env string :=
  ...
  | [] => ret []
  | (name, decl) :: Σ =>
    if eq_kername name ChainHelpers_kername then
      let rep_term := tConst axiomatized_ChainHelpers_kername [] in
      ...
      ret ((name, decl)
              :: (axiomatized_ChainHelpers_kername, axiomatized_ChainHelpers_decl)
              :: rev newΣ_rev)
    else
...
Definition specialize_ChainHelpers_env (Σ : global_env) : result global_env string :=
