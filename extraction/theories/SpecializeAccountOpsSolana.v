...
Definition AccountOperations_kername : kername :=
  <%% @AccountGetters %%>.
    ...
    | ConstantDecl cst =>
      let remove := match cst_type cst with
                    | tProd _ (tInd ind _) _ =>
                      eq_kername (inductive_mind ind) (AccountOperations_kername)
                    | _ => false
                    end in
...
Definition axiomatized_AccountOperations := 0.

Definition axiomatized_AccountOperations_kername : kername :=
  <%% axiomatized_AccountOperations %%>.

Definition axiomatized_AccountOperations_decl : global_decl :=
  ConstantDecl
    {| cst_type :=
          tInd
            {| inductive_mind := AccountOperations_kername;
              inductive_ind := 0; |}
            [];
        cst_body := None;
...
Fixpoint specialize_env_rev (Σ : global_env) : result global_env string :=
  ...
  | [] => ret []
  | (name, decl) :: Σ =>
    if eq_kername name AccountOperations_kername then
      let rep_term := tConst axiomatized_AccountOperations_kername [] in
      ...
      ret ((name, decl)
              :: (axiomatized_AccountOperations_kername, axiomatized_AccountOperations_decl)
              :: rev newΣ_rev)
    else
...
Definition specialize_AccountOperations_env (Σ : global_env) : result global_env string :=
