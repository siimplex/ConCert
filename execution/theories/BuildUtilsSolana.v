...
(* J> TODO: Correct naming in proof window *)
...
Proof.
  ...
  - destruct_chain_step;
    ...
    now rewrite IH in act_eq by assumption.
Qed.
...
Proof.
  ...
  - destruct_chain_step;
      ...
      cbn in *;
      destruct_address_eq; now subst.
      + try (setoid_rewrite new_acts_eq);
      cbn in *;
      destruct_address_eq; now subst.
      + try (setoid_rewrite e);
      cbn in *;
      destruct_address_eq; now subst.
      + try (setoid_rewrite deployed_state1);
      cbn in *;
      destruct_address_eq; now subst.
      + try (setoid_rewrite env_eq);
      cbn in *;
...
Proof.
  ...
  - destruct_chain_step; auto.
    ...
    apply Z.ge_le. rewrite enough_balance. easy. 
Qed.
...
Definition receiver_can_receive_transfer (bstate : ChainState) act_body :=
  ...
  | act_transfer to _ => address_is_contract to = false \/
    (exists wc state,
      ...
      forall (bstate_new : ChainState) accs, exists new_state, wc_process wc bstate_new accs (Some state) None = Some (new_state, []))
  | _ => True
  end.
...
Lemma wc_receive_to_receive : forall {Msg State Setup : Type}
                                    `{Serializable Msg}
                                    `{Serializable State}
                                    `{Serializable Setup}
                                    (contract : Contract Msg State Setup)
                                    chain accounts cstate msg new_cstate new_acts,
  contract.(process) chain accounts (Some cstate) (Some msg) = Some (new_cstate, new_acts) <->
  wc_process contract chain accounts (Some ((@serialize State _) cstate)) (Some ((@serialize Msg _) msg)) = Some ((@serialize State _) new_cstate, new_acts).
Proof.
  ...
  - apply wc_receive_strong in receive_some as
    ...
    destruct prev_state' eqn:Hstate.
    + cbn in prev_state_eq.
...
Lemma evaluate_action : forall {Setup Msg State Setup : Type}
                              `{Serializable Msg}
                              `{Serializable State}
                              `{Serializable Setup}
                               (contract : Contract Msg State Setup)
  ...
  BlockchainSolana.process contract (transfer_balance from caddr amount bstate)
                     accounts
                     (Some cstate) (Some msg) = Some (new_cstate, new_acts) ->
    (exists bstate',
       reachable_through bstate bstate'
...
(*   all: swap 2 3. *)
  ...
  - eapply step_action; eauto.
    ...
    + now apply wc_receive_to_receive in process_some.
...
Lemma deploy_contract : forall {Msg State Setup : Type}
                              `{Serializable Msg}
                              `{Serializable State}
                              `{Serializable Setup}
                               (contract : Contract Msg State Setup)
                               bstate origin from owner caddr amount accounts acts msg setup cstate,
  reachable bstate ->
  chain_state_queue bstate = {| act_from := from;
                                act_origin := origin;
                                act_body := act_deploy contract ((@serialize Msg _) msg) |} :: acts ->
  amount >= 0 ->
  env_account_balances bstate from >= amount ->
  address_is_contract caddr = true ->
  env_contracts bstate caddr = None ->
  env_account_owners bstate caddr = Some owner ->
  BlockchainSolana.process contract
        bstate
        accounts
        None
        (Some msg) = Some (cstate, []) ->
...
Proof.
  intros * reach queue amount_nonnegative enough_balance%Z.ge_le
    caddr_is_contract not_deployed caddr_owner init_some.
  pose (bstate' := (bstate<|chain_state_queue := acts|>
                          <|chain_state_env :=
                            (set_contract_state caddr ((@serialize State _) cstate)
                            (add_contract caddr contract bstate))|>)).
  ...
  - eapply step_action with (new_acts := []); eauto.
    ...
    + apply wc_init_to_init in init_some. rewrite <- enough_balance. apply Z.ge_le. easy.
    + apply wc_init_to_init in init_some. easy.
    + constructor; try reflexivity. 
  - exists bstate'.
